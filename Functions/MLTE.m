...
%Remarque : prio de AppData et non pas de A
%modifier et voir le resultat <<<< important <<<<
    ...
    for m = 1 : 5
        ...
        if( getsize(a,3) == 3) %on a 3 classes
            ...
            for i = 1 : getsize(a,2)
                ...
                f(:,i,1) = prio(1)*ksdensity(appData(getnlab(app)==1,i),testData(:,i));
                
                f(:,i,2) = prio(2)*ksdensity(appData(getnlab(app)==2,i),testData(:,i));
                
                f(:,i,3) = prio(3)*ksdensity(appData(getnlab(app)==3,i),testData(:,i));
                
                f(:,i,4) = prio(4)*ksdensity([appData(getnlab(app)==1,i);appData(getnlab(app)==2,i)],testData(:,i));
                
                f(:,i,5) = prio(5)*ksdensity([appData(getnlab(app)==1,i);appData(getnlab(app)==3,i)],testData(:,i));
                
                f(:,i,6) = prio(6)*ksdensity([appData(getnlab(app)==2,i);appData(getnlab(app)==3,i)],testData(:,i));
                
                f(:,i,7) = prio(7)*ksdensity([appData(getnlab(app)==1,i);appData(getnlab(app)==2,i);appData(getnlab(app)==3,i)],testData(:,i));
                ...
                for i = 1 : getsize(a,2)
                    ...
                    f(:,i,3) = prio(3)*ksdensity([appData(getnlab(app)==1,i);appData(getnlab(app)==2,i)],testData(:,i));
